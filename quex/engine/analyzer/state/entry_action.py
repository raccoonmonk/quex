...
class TransitionID(namedtuple("TransitionID_tuple", ("target_state_index", "source_state_index", "trigger_id"))):
    ...
    def __new__(self, StateIndex, FromStateIndex, TriggerId):
        assert isinstance(StateIndex, int)     or StateIndex     in E_StateIndices
        assert isinstance(FromStateIndex, int) or FromStateIndex in E_StateIndices
        assert isinstance(TriggerId, int)      
