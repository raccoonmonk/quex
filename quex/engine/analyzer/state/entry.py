...
class Entry(object):
    ...
    def get_transition_id_list(self, DoorId):
        return [ 
           transition_id for transition_id, action in self.__db.items()
    ...
    def absorb(self, Other):
        ...
        """
        for tid, action in Other.__db.items():
    ...
    def enter(self, ToStateIndex, FromStateIndex, TheAction):
        ...
        #!! DO NOT EVEN THINK ABOUT DELETING THIS ASSERT
        assert all(id(TheAction.command_list) != id(action.command_list) 
                   for transition_id, action in self.__db.items())
    ...
    def add_StoreInputPosition_on_all(self, AccConditionSet, PositionRegister, Offset):
        """Add an acceptance at the top of each accepter at every door. If there
           is no accepter in a door it is created.
        """
        cmd = Op.StoreInputPosition(AccConditionSet, PositionRegister, Offset)
        for ta in self.__db.values():
            ta.command_list.insert_StoreInputPositionUniquely(cmd.clone())

    def append_Op_on_all(self, op):
        """Add an acceptance at the top of each accepter at every door. If there
           is no accepter in a door it is created.
        """
        for ta in self.__db.values():
            ta.command_list.append(op)
    ...
    def add_StoreInputPosition(self, StateIndex, FromStateIndex, AccConditionSet, PositionRegister, Offset):
        ...
        ####### 'command_list.insert_StoreInputPositionUniquely(cmd)'
    ...
    def has_command(self, OpId):
        assert OpId in E_Op
        for action in self.__db.values():
    ...
    def categorize(self, StateIndex):
        ...
        #!! independent! Each transition must have its OWN OpList!
        cmd_list_ids = [ 
            id(action.command_list) for action in self.__db.values() 
        ]
        assert len(cmd_list_ids) == len(set(cmd_list_ids)) # size(list) == size(unique set)

        work_list = [
            (transition_id, action) for transition_id, action in self.__db.items()
                                    if action.door_id is None
        ]
        if not work_list: return

        command_list_db = dict(
            (action.command_list, action.door_id) for action in self.__db.values()
                                                  if action.door_id is not None
        )
        ...
        for transition_id, action in sorted(work_list, key=sort_key): 
    ...
    def check_consistency(self):
        ...
        # --> Op.unique_set
        for transition_action in self.__db.values():
    ...
    def __repr__(self):
        ...
        result = []
        for transition_id, door in sorted(iter(self.__db.items()),key=lambda x: x[0].source_state_index):
            accept_command_list = []
            store_command_list  = []
            pcok_command_list   = []
            ssk_command_list    = []
