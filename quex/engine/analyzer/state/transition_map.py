...
class TransitionMap(list):
    ...
    def relate_to_DoorIDs(self, TheAnalyzer, StateIndex):
        ...
        def relate(Target):
            ...
            assert door_id is not None, \
                    ...
                    (Target, StateIndex, Target, [tid for tid, ta in te.items()])
            return door_id
    ...
    def match_with_wildcard(self, Other, ExceptionCharacter=None, EqualCmp=lambda x,y: x==y):
        ...
        DoorId,  the target that the wild card shall hit so that the
        ...
        assert isinstance(wildcard_target, DoorID) or wildcard_target == -1
    ...
    def combine_adjacents(self, Index=None):
        ...
        if Index is None:
            range_iterable = reversed(range(L))
        else:
        ...
        prev_interval, prev_target = self[next(range_iterable)]
    ...
    def assert_continuity(self, StrictF=True):
        ...
        iterable                   = iter(self)
        prev_interval, prev_target = next(iterable)
        # No 'empty' intervals
