 ...
 * refuses to operate. 
 *
 * The new chunk is allocated with 'E_Ownership_LEXER', such that 
 ...
 *          false, else.                                                      */
{
    ...
    if( ! new_memory_root_p ) {
        ...
        root->_memory.ownership = E_Ownership_LEXER;
        verdict_f = true;
    }
    QUEX_NAME(Buffer_assert_consistency)(me);
    return verdict_f;
 ...
 *          false, if newly allocated memory is too small.                    */
{
    ...
    if( required_size > MemoryLexatomN ) {
        ...
        QUEX_GSTD(memcpy)((void*)&memory[0], 
        ...
        if( root->_memory.ownership == E_Ownership_LEXER ) {
 ...
 *                                                                            */
{
    ...
    /* LexatomBeforeLexemeStart       */ $$<begin-of-line-context>     me->_lexatom_before_lexeme_start,$$
...
#endif /* QUEX_INCLUDE_GUARD__BUFFER__BUFFER_NESTED_I */
