...
#ifndef  QUEX_INCLUDE_GUARD__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I
#define  QUEX_INCLUDE_GUARD__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I
 ...
 *    -- QUEX_GNAME_LIB(ByteLoader)'s seek sets the position in the low level input
 *       stream.
 * 
 * A LexatomLoader's seek has NO direct influence on the content or the pointers
 * of the Buffer object that it fills.
 *
 * The difference between the QUEX_GNAME_LIB(ByteLoader)'s seeking (stream seeking) 
 * and the LexatomLoader's seeking is that the QUEX_GNAME_LIB(ByteLoader) understands 
 * the position independently of its meaning. The LexatomLoader already 
 * interprets the stream as 'lexatoms'. A QUEX_GNAME_LIB(ByteLoader) sets the stream 
...
{
    if( me->byte_loader ) {
        me->byte_loader->seek(me->byte_loader, 0);
        if( me->byte_loader->tell(me->byte_loader) != (QUEX_TYPE_STREAM_POSITION)0 ) {
            QUEX_ERROR_EXIT("QUEX_GNAME_LIB(ByteLoader )failed to seek to initial position.\n");
            return false;
        }
    }
...
QUEX_NAME(LexatomLoader_lexatom_index_reset_backup)(QUEX_NAME(LexatomLoader)* me, 
                                                    ...
                                                    ptrdiff_t                 BackupStomachByteN, 
                                                    QUEX_TYPE_STREAM_POSITION BackupByteLoaderPosition)
/* Reset a previous state of the LexatomLoader and its QUEX_GNAME_LIB(ByteLoader).            */
{
...
/* From the given 'lexatom_index_next_to_fill' step forward to lexatom index 
 * 'TargetCI'. This function is used to reach a target input position in cases 
 * where computing is impossible.
 *
...
#endif /*   QUEX_INCLUDE_GUARD__BUFFER__LEXATOMS__LEXATOM_LOADER_NAVIGATION_I */
