 ...
 * RETURNS: Number of free lexatoms from 'content_end' the end of buffer.     */
{
    ...
    if( MoveDistance ) {
        ...
        if( MoveDistance && move_size ) {
            QUEX_GSTD(memmove)((void*)me->content_begin(me), 
 ...
 *          buffer's content begin.                                           */
{
    ...
    if( MoveDistance ) {
        ...
        if( MoveDistance <= ContentSpaceSize ) {
            ...
            if( MoveDistance && move_size ) {
                QUEX_GSTD(memmove)((void*)&BeginP[MoveDistance], BeginP, 
                                   (size_t)move_size * sizeof(QUEX_TYPE_LEXATOM));
            }
 ...
 * That is, they indicate the situation to be restored.                       */
{
    ...
    if( move_size && MoveDistance ) {
        ...
        QUEX_GSTD(memmove)((void*)&BeginP[MoveDistance], (void*)BeginP, 
                           (size_t)move_size * sizeof(QUEX_TYPE_LEXATOM));
 ...
 *        begin of the buffer.                                                */
{
    ...
    move_distance_max = QUEX_MIN((ptrdiff_t)me->input.lexatom_index_begin, 
    ...
    if( *lexatom_index_to_be_contained < lexatom_index_begin ) {
        ...
        if( *lexatom_index_to_be_contained > me->input.lexatom_index_end_of_stream ) {
            ...
            if( *lexatom_index_to_be_contained == lexatom_index_end ) {
                ...
                *lexatom_index_to_be_contained = QUEX_MAX((ptrdiff_t)0, 
                                                          (ptrdiff_t)(*lexatom_index_to_be_contained - (QUEX_TYPE_STREAM_POSITION)1));
            }
        }
    }

    new_lexatom_index_begin = QUEX_MAX((ptrdiff_t)0, 
                                       (ptrdiff_t)(*lexatom_index_to_be_contained - (QUEX_TYPE_STREAM_POSITION)FallBackN));
...
#endif /* QUEX_INCLUDE_GUARD__BUFFER__BUFFER_MOVE_I */
