...
#ifndef QUEX_INCLUDE_GUARD__ANALYZER__STRUCT__RESET_I
#define QUEX_INCLUDE_GUARD__ANALYZER__STRUCT__RESET_I
 ...
 *          false, in case of failure.                                        */
{
    ...
    else if( ! QUEX_NAME(MF_reset_ByteLoader)(me, new_byte_loader, new_converter) ) {
        goto ERROR_1;
    }
 ...
 *          false, in case of failure.                                        */
{
    ...
    if( me->error_code != E_Error_None ) {
        QUEX_NAME(MF_error_code_set_if_first)(me, E_Error_Reset_OnError);
        goto ERROR_0;
    }
    ...
    if( me->buffer.filler ) {
        me->buffer.filler->destruct(me->buffer.filler);
        QUEX_GNAME_LIB(MemoryManager_free)((void*)me->buffer.filler, 
...
ERROR_0:
    ...
    if( new_converter ) {
        new_converter->destruct(new_converter); 
        QUEX_GNAME_LIB(MemoryManager_free)((void*)new_converter, E_MemoryObjectType_BUFFER_FILLER);
    }
    /* 'destruct' marks resources as absent => double destruction is safe.    */
ERROR_1:
 ...
 *          false, else. Buffer's resource must be destructed/marked absent.  */
{
    ...
    if( ! QUEX_NAME(user_reset)(me) ) {
        QUEX_NAME(MF_error_code_set_if_first)(me, E_Error_UserReset_Failed);
        goto ERROR_0;
    }
...
#endif /* QUEX_INCLUDE_GUARD__ANALYZER__STRUCT__RESET_I */
