...
def do_shortcut(stream, ShortcutLetter, PropertyAlias):
    r"""Name property shortcut '\ShortcutLetter{...}' which is a shortcut
    for '\P{PropertyAlias=...}'.
...
def __parse_property_expression(stream, PropertyLetter, EqualConditionPossibleF=True):
    r"""Parses an expression of the form '\? { X [ = Y] }' where
    ...
    x = stream.read(2)
    if x != "\\" + PropertyLetter: 
        raise RegularExpressionException("Unicode property letter '\\%s' expected, received '%s'." % (PropertyLetter, x))
